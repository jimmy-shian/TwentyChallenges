//$(document).ready(function() {
    ...
    function checkscrollBtnSize() {
        var windowWidth = $(window).width();
        var scrollBtn = $("#scrollBtn");

        if (windowWidth <= 825) {
            scrollBtn.css('display', 'none'); // 小於825px，隱藏
            butt_scroll = false;
        } else {
            scrollBtn.css('display', ''); // 大於825px，顯示
            butt_scroll = true;
        }
    }
    ...
    function checkWindowSize() {
        ...
        if (windowWidth <= 716) {
            overlay.css('display', 'none'); // 小於716px，隱藏
        } else {
            overlay.css('display', 'flex'); // 大於716px，顯示
        }
    }
    ...
    function handleScrollButton() {
        ...
        if (scrollPosition >= windowHeight / 2) {
            ...
            btn.html("<box-icon type='solid' name='chevrons-up'></box-icon>"); // 上
            btn.off('click').on('click', function() {
                $('html, body').animate({ scrollTop: 0 }, 'smooth'); // 滾動到頂部
            });
        } else {
            btn.css('display', 'block'); // 顯示按鈕
            btn.html("<box-icon type='solid' name='chevrons-down'></box-icon>"); // 下
            btn.off('click').on('click', function() {
...
//});
    ...
    function fillRandomAdventures() {
        const randomAdventures = [
            ...
            // 創意靜態類
            '讀書', '下棋', '瑜伽', '靜坐冥想', '看電影', '繪畫', '手工藝', '做麵包', '園藝種花', 
            '散步', '寫作', '聽音樂', '攝影',
            ...
            // 社交活動類
            '聚會', '野餐', '野炊', '桌遊之夜', '參加音樂會', '參加講座', '公益活動', '博物館參觀',
